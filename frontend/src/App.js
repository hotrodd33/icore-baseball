  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:5000/api/get_both_stats',
        {
          batter: {
      ...
      if (response.data.pitcher) {
        if (response.data.pitcher.error) {
          setError((prev) =>
            prev
              ? `${prev}, Pitcher Error: ${response.data.pitcher.error}`
              : `Pitcher Error: ${response.data.pitcher.error}`
          )
        } else {
    ...
    const rollInterval = setInterval(() => {
      ...
      if (counter >= totalSteps) {
        ...
        if (pitcherResults?.count_frequencies) {
          const matchedCount = pitcherResults.count_frequencies.find(
            (count) =>
              finalRandom1 >= count.range_start &&
              finalRandom1 <= count.range_end
          )
          ...
          if (matchedCount) {
            ...
            const activeCard =
              finalRandom2 < preference.pitcher ? 'pitcher' : 'batter'
            ...
            const results =
              activeCard === 'pitcher' ? pitcherResults : batterResults
            if (results?.event_ranges) {
              const leftyEvents = results.event_ranges?.lefty || []
              const rightyEvents = results.event_ranges?.righty || []

              const matchingLeftyEvents = leftyEvents
                .filter(
                  (event) =>
                    event.count_label === countLabel &&
                    finalRandom3 >= event.range_start &&
                    finalRandom3 <= event.range_end
                )
                .map((event) => event.event)

              const matchingRightyEvents = rightyEvents
                .filter(
                  (event) =>
                    event.count_label === countLabel &&
                    finalRandom3 >= event.range_start &&
            ...
            <div className="form-section batter-form">
              ...
              <div className="pagination-controls">
                ...
                <button
                  type="button"
                  onClick={nextBatter}
                  disabled={currentBatterIndex === testData.batters.length - 1}
                  className="pagination-button"
                >
                  →
                </button>
              ...
              <input
                ...
                value={batterFirstName}
                onChange={(e) => setBatterFirstName(e.target.value)}
              />
              <input
                type="text"
                placeholder="Last Name"
                value={batterLastName}
                onChange={(e) => setBatterLastName(e.target.value)}
              />
              <input
                type="text"
                placeholder="Year"
                value={batterYear}
                onChange={(e) => setBatterYear(e.target.value)}
              />
            </div>
            ...
            <div className="form-section pitcher-form">
              ...
              <div className="pagination-controls">
                <button
                  type="button"
                  onClick={prevPitcher}
                  disabled={currentPitcherIndex === 0}
                  className="pagination-button"
                >
                  ←
                </button>
                ...
                <button
                  ...
                  onClick={nextPitcher}
                  disabled={
                    currentPitcherIndex === testData.pitchers.length - 1
                  }
                  className="pagination-button"
                >
                  →
              ...
              <input
                type="text"
                placeholder="First Name"
                value={pitcherFirstName}
                onChange={(e) => setPitcherFirstName(e.target.value)}
              />
              <input
                type="text"
                placeholder="Last Name"
                value={pitcherLastName}
                onChange={(e) => setPitcherLastName(e.target.value)}
              />
              <input
                type="text"
                placeholder="Year"
                value={pitcherYear}
                onChange={(e) => setPitcherYear(e.target.value)}
              />
            </div>
        ...
        {(batterResults || pitcherResults) && (
          <div className="event-range-container">
            <div className="event-range-tables">
              <div className="count-result-container">
                <div className="count-result">
                  <CountFrequenciesTable
                    data={pitcherResults?.count_frequencies || []}
                    highlightedCount={highlightedCount}
                    randomRoll={firstRandom}
                  />
                </div>
                <div className="card-preference">
                  <CardPreferenceTable
                    cardPreferences={cardPreferences}
                    randomRoll={secondRandom}
                    highlightedCount={highlightedCount}
                  />
                </div>
              </div>
              ...
              <div
                className={`result-tables-container ${
                  activeCard === 'pitcher' ? 'pitcher-active' : 'batter-active'
                }`}
              >
                {/* Pitcher Results */}
                <div
                  className={`player-results pitcher-results ${
                    activeCard === 'pitcher' ? 'active-card' : 'inactive-card'
                  }`}
                >
                  <h2>
                  ...
                  <EventRangeTable
                    data={pitcherResults?.event_ranges?.righty || []}
                    title={`vs RH Batters`}
                    highlightedCount={highlightedCount}
                    highlightedEvents={highlightedEventsRighty}
                    highlightedRoll={thirdRandom}
                  />
                  <EventRangeTable
                    data={pitcherResults?.event_ranges?.lefty || []}
                    title={`vs LH Batters`}
                    highlightedCount={highlightedCount}
                    highlightedEvents={highlightedEventsLefty}
                    highlightedRoll={thirdRandom}
                  />
                </div>
                ...
                {/* Batter Results */}
                <div
                  className={`player-results batter-results ${
                    activeCard === 'batter' ? 'active-card' : 'inactive-card'
                  }`}
                >
                  <h2>
                  ...
                  <EventRangeTable
                    data={batterResults?.event_ranges?.righty || []}
                    title={`vs RH Pitchers`}
                    highlightedCount={highlightedCount}
                    highlightedEvents={highlightedEventsRighty}
                    highlightedRoll={thirdRandom}
                  />
                  <EventRangeTable
                    data={batterResults?.event_ranges?.lefty || []}
                    title={`vs LH Pitchers`}
                    highlightedCount={highlightedCount}
                    highlightedEvents={highlightedEventsLefty}
                    highlightedRoll={thirdRandom}
                  />
